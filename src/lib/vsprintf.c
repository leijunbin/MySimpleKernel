...
static char *number(char *str, unsigned long num, int base, int size,
  ...
  // 如果falgs指出用小写字母，则定义小写字母集
  if (flags & SMALL) digits = "0123456789abcdefghijklmnopqrstuvwxyz";

  // 如果falgs指出要左对齐，则屏蔽类型中的填零标志
  if (flags & LEFT) flags &= ~ZEROPAD;

  // 如果进制基数小于2或大于36，则退出处理
  if (base < 2 || base > 36) return 0;

  // 如果flags指出要填零，则置字符变量c='0'，否则c等于空格字符
  c = (flags & ZEROPAD) ? '0' : ' ';

  // 如果flags指出是带符号数并且数值num小于0，则置符号变量
  // sign=负号，并使num取绝对值
  // 否则如果flags指出是加号，则置sign=加号
  // 否则若类型带空格标志则sign=空格，否则置0
  if (flags & SIGN && num < 0) {
    sign = '-';
    num = -num;
  } else {
  ...
  // 若flags指出是特殊转换，则对于十六进制宽度再减少2位(用于0x)
  if (flags & SPECIAL) {
    if (base == 16) size -= 2;
  ...
  // 若flags中没有填零(ZEROPAD)和左对齐（左调整）标志
...
int vsprintf(char *buf, const char *fmt, va_list args) {
  ...
  int precision;    // min整数数字个数;max字符串中字符
  ...
  repeat:
    ...
    switch (*fmt) {
      ...
      case 'o':
        str =
            number(str, va_arg(args, int64), 8, field_width, precision, flags);
        break;
      ...
      case 'p':
        ...
        str = number(str, (long)va_arg(args, void *), 16, field_width,
                     precision, flags);
        break;
      ...
      case 'X':
        str =
            number(str, va_arg(args, int64), 16, field_width, precision, flags);
        break;
      ...
      case 'u':
        str =
            number(str, va_arg(args, int64), 10, field_width, precision, flags);
        break;
